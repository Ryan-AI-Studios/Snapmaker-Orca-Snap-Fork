              ...
              std::vector<int> *upper_source_face, std::vector<int> *lower_source_face)
{
    ...
    for (int facet_idx = 0; facet_idx < int(mesh.indices.size()); ++ facet_idx) {
        ...
        auto push_mapped = [](indexed_triangle_set *its, std::vector<int> *src, const stl_triangle_vertex_indices &f, int src_idx) {
            if (its == nullptr)
                return;
            its->indices.emplace_back(f);
            if (src)
                src->push_back(src_idx);
        };

        if (min_z > z || (is_equal(min_z , z) && max_z > z)) {
            // facet is above the cut plane and does not belong to it
            push_mapped(upper, upper_source_face, facet, facet_idx);
        } else if (max_z < z || (is_equal(max_z, z) && min_z < z)) {
            // facet is below the cut plane and does not belong to it
            push_mapped(lower, lower_source_face, facet, facet_idx);
            ...
            auto new_face = [upper_source_face, lower_source_face, facet_idx](indexed_triangle_set *its, std::vector<int> *src, int i, int j, int k) {
                if (its != nullptr && i != j && i != k && j != k) {
                    its->indices.emplace_back(i, j, k);
                    if (src)
                        src->push_back(facet_idx);
                }
            };
            if (is_new_vertex_v0v1 && is_new_vertex_v2v0) {
                if (v0.z() > z) {
                    new_face(upper, upper_source_face, iv0, iv0v1_upper, iv2v0_upper);
                    new_face(lower, lower_source_face, iv1, iv2, iv0v1_lower);
                    new_face(lower, lower_source_face, iv2, iv2v0_lower, iv0v1_lower);
                } else {
                    new_face(upper, upper_source_face, iv1, iv2, iv0v1_upper);
                    new_face(upper, upper_source_face, iv2, iv2v0_upper, iv0v1_upper);
                    new_face(lower, lower_source_face, iv0, iv0v1_lower, iv2v0_lower);
                }
            } else if (is_new_vertex_v0v1) {
                if (v0.z() > z) {
                    new_face(upper, upper_source_face, iv0, iv0v1_upper, iv2);
                    new_face(lower, lower_source_face, iv1, iv2, iv0v1_lower);
                } else {
                    new_face(lower, lower_source_face, iv0, iv0v1_lower, iv2);
                    new_face(upper, upper_source_face, iv1, iv2, iv0v1_upper);
                }
            } else if (is_new_vertex_v2v0) {
                if (v0.z() > z) {
                    new_face(upper, upper_source_face, iv0, iv1, iv2v0_upper);
                    new_face(lower, lower_source_face, iv1, iv2, iv2v0_lower);
                } else {
                    new_face(lower, lower_source_face, iv0, iv1, iv2v0_lower);
                    new_face(upper, upper_source_face, iv1, iv2, iv2v0_upper);
                }
            }
        }
    }
